...
jazzHandler.handle = async function(component, scriptPath, parameter) {
	let jazzTemplate = await fs.readFile(process.cwd() + scriptPath, {encoding: 'utf8'});
	logger.trace("Handling jazz file [" + scriptPath + "]");
	return await jazzHandler.handleJazzTemplate(component, jazzTemplate, parameter);
};

jazzHandler.handleJazzTemplate = async function(component, jazzTemplate, parameter) {
...
jazzHandler.includeComponent = function(resourcePath, componentPath, cb, component, selector) {
	...
	dispatcher.dispatchComponent(resourcePath, componentPath, component, selector)
	.then(result => {
		cb(result);
	}).catch(err => {
		let errorMessage = `Error while including component '${componentPath}' with resource path '${resourcePath}`;
...
jazzHandler.includeScript = function(scriptFilename, cb, component, parameter) {
	...
	component.executeScript(scriptName, scriptExtension, parameter).then(cb).catch(function(err) {
		let errorMessage = `Error while including script '${scriptFilename}' within component '${component.componentResource.path}`;
		logger.error(errorMessage, err);
		cb(errorMessage);
	});
};
