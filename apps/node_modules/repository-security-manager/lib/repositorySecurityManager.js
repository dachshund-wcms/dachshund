...
securityManager.getAuthorizationList = function(resource) {
	let deferred = Q.defer();

	let allowList = {};
	let denyList = {};

	loadAuthorizationList(resource, allowList, denyList).then(function() {
		deferred.resolve([allowList, denyList]);
	}).fail(function(error) {
		logger.error("Error while loading authorization list: " + error.toString());
	});

	return deferred.promise;
};

const loadAuthorizationList = function(resource, allowList, denyList) {
	let deferred = Q.defer();
	...
	if (resource.depth > 2)
	{
		resource.getParentResource().then(function(parentResource) {
			loadAuthorizationList(parentResource, allowList, denyList).then(deferred.resolve).fail(deferred.reject);
