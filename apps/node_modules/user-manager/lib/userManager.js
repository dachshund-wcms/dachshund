...
userManager.load = async function(username) {
	let userResourcePath = USERS_DIRECTORY + username;
	let userResource = await RepositoryManager.resolve(userResourcePath);
...
userManager.authenticate = async function(username, password) {
	let userResource = await userManager.load(username);
